...
const mapToOutput = (data: EntryFormData): CreateEntryData => {
    const definitions: CreateDefinitionData[] = data.definitions.map((d) => ({
        definitionText: d.definitionText,
        source: d.source,
        examples: d.examples.map(
            (ex): CreateExampleData => ({
    ...
    const translations: CreateTranslationData[] = data.translations.map(
        (t) => ({
