...
**Scope:** `Wordfolio.Api.Domain` project only.

These rules govern the internal design of the domain layer: how features are structured, how capabilities and operations are defined, and how errors are modeled. They complement but do not replace the project-wide conventions in the root `AGENTS.md`.
...
- Keep shared-level files at the domain root as `Types.fs`, `Capabilities.fs`, and `Operations.fs`.
...
- If a feature needs another feature's concept, move that concept (type/capability/helper operation) to root-level shared files.
