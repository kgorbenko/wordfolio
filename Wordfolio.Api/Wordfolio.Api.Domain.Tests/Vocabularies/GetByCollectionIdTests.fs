...
type TestEnv
    ...
    let getVocabulariesByCollectionIdCalls = ResizeArray<CollectionId>()

    member _.GetCollectionByIdCalls = getCollectionByIdCalls |> Seq.toList
    member _.GetVocabulariesByCollectionIdCalls = getVocabulariesByCollectionIdCalls |> Seq.toList
...
let ``returns vocabularies when collection owned by user`` () =
    ...
    let vocabularies =
        [ makeVocabulary 1 1 "Vocabulary 1"
          makeVocabulary 2 1 "Vocabulary 2" ]
    ...
    task {
        ...
        Assert.Equal([ CollectionId 1 ], env.GetVocabulariesByCollectionIdCalls)
    }
